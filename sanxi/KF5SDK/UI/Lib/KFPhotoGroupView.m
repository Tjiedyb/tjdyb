...
- (void)setItem:(KFPhotoGroupItem *)item {
   ...
   [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:item.largeImageURL options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
               if(!weakSelf)return;
...
//    [[SDWebImageManager sharedManager] downloadImageWithURL:item.largeImageURL options:kNilOptions progress:^(nullable SDWebImageDownloaderProgressBlock) {
////        if(!weakSelf)return;
////        CGFloat progress = receivedSize / (float)expectedSize;
////        progress = progress < 0.01 ? 0.01 : progress > 1 ? 1 : progress;
////        if (isnan(progress)) progress = 0;
